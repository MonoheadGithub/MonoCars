...
const slides = [
...

  {
    ...
    desc: "★ Premium Only ★ Acceleration:Good / Brakes:Okay / Handling:Okay",
    color: "#FFC300",
  ...
  {
    id: 2,
    name: "★ 1969 Ferrari 312",
    desc: "★ Premium Only ★ Acceleration:Good / Brakes:Good / Handling:Great",
    color: "#FFC300",
    imgFloorUrl: floorImg,
    imgUrl: mis2,
    topSpeed: 120,
  ...
  return /*#__PURE__*/ (
    React.createElement("div", {
      ...
      React.createElement("h1", {
        ...
        React.createElement("span", null, "Miscellaneous:"), /*#__PURE__*/
...
class Slider extends React.Component {
  ...
  componentDidMount() {
    ...
    if (window.location.href.indexOf("?=2013PorscheGT3RSR") > -1) {
      this.setActiveSlide(this.state.activeSlide + 0);

    }

    if (window.location.href.indexOf("?=1969Ferrari312") > -1) {
      this.setActiveSlide(this.state.activeSlide + 1);

    }

    if (window.location.href.indexOf("?=1969HotwheelsTwinMill") > -1) {
      this.setActiveSlide(this.state.activeSlide + 2);

    }

    if (window.location.href.indexOf("?=2005MazdaFurai") > -1) {
      this.setActiveSlide(this.state.activeSlide + 3);

    }

    if (window.location.href.indexOf("?=MorganAeroSS") > -1) {
      this.setActiveSlide(this.state.activeSlide + 4);

    }

    if (window.location.href.indexOf("?=1963MercedesBenz300SL") > -1) {
      this.setActiveSlide(this.state.activeSlide + 5);

    }

    if (window.location.href.indexOf("?=1940FordDeluxCoupe") > -1) {
      this.setActiveSlide(this.state.activeSlide + 6);

    }

    if (window.location.href.indexOf("?=2003PorscheTricycle") > -1) {
      this.setActiveSlide(this.state.activeSlide + 7);

    }

    if (window.location.href.indexOf("?=WheelChair") > -1) {
