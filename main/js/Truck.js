...
class Slider extends React.Component {
  ...
  componentDidMount() {
    ...
    if (window.location.href.indexOf("?=Peterbilt579Truck") > -1) {
      this.setActiveSlide(this.state.activeSlide + 0);

    }

    if (window.location.href.indexOf("?=2012FordRaptor") > -1) {
      this.setActiveSlide(this.state.activeSlide + 1);

    }

    if (window.location.href.indexOf("?=2014MercedesSprinterBoxTruck") > -1) {
      this.setActiveSlide(this.state.activeSlide + 2);

    }

    if (window.location.href.indexOf("?=2008FordF350Superduty") > -1) {
      this.setActiveSlide(this.state.activeSlide + 3);

    }

    if (window.location.href.indexOf("?=1969ChevroletC10") > -1) {
      this.setActiveSlide(this.state.activeSlide + 4);

    }

    if (window.location.href.indexOf("?=2009ToyotaTundraCrewmax") > -1) {
